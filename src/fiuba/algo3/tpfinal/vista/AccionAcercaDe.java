...
public class AccionAcercaDe implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		informacion.setBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(51, 51, 51), null));
		informacion.setBackground(new Color(0, 0, 0));
		informacion.setFont(new Font("Stencil", informacion.getFont().getStyle(), informacion.getFont().getSize() + 13));
		panelUno.add(informacion);
		
		JPanel panelDos = new JPanel();
		JLabel datos = new JLabel("Los alumnos que crearon este juego son:");
		datos.setFont(new Font("Stencil", datos.getFont().getStyle(), datos.getFont().getSize() + 9));
		panelDos.add(datos);
		
		JPanel panelTres = new JPanel();
		JLabel creadores = new JLabel("Agustin, Damian y Luciano");
		creadores.setFont(new Font("Sylfaen", creadores.getFont().getStyle(), creadores.getFont().getSize() + 9));
